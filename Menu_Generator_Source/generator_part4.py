...
<script>
  (function() {
    // Theme is handled entirely by CSS prefers-color-scheme.
    // No localStorage persistence - site always matches the system setting.
  })();
</script>
</head>
<body>
...
<script>
  ...
  function toggleTheme() {
    ...
    // If no override set, determine effective theme from system
    if (!currentTheme) {
      currentTheme = window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light';
    }
    const newTheme = currentTheme === 'dark' ? 'light' : 'dark';
    document.documentElement.setAttribute('data-theme', newTheme);
    // No localStorage - preference resets to system on next page load
  }
...
with open("generator_core.py", "w", encoding="utf-8") as f:
    ...
    f.write('from generator_part5 import MENU_DATA as d5\n')
    f.write('import json\n')
